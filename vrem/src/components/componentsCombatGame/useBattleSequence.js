...
import { wait, attack, magic, heal } from './Helper'
      ...
      if (mode) {
        ...
        switch (mode) {
          case 'attack': {
            ...
            (async () => {
              ...
              turn === 0
                ? setOpponentAnimation('static')
                : setPlayerAnimation('static');
              setAnnouncerMessage(`${receiver.name} stumbles backwards!`);
              turn === 0
          ...
          case 'magic': {
            ...
            (async () => {
              setInSequence(true);
              setAnnouncerMessage(`${attacker.name} has cast a cantrip!`);
              await wait(1000);
              ...
              turn === 0
                ...
                : setPlayerAnimation('static');
              setAnnouncerMessage(
                `${receiver.name} tingles from the eldritch blast!`,
              );
              turn === 0
                ? setOpponentHealth(h => (h - damage > 0 ? h - damage : 0))
                : setPlayerHealth(h => (h - damage > 0 ? h - damage : 0)); // We don't want a negative HP.
          ...
          case 'heal': {
            ...
            (async () => {
              setInSequence(true);
              setAnnouncerMessage(`${attacker.name} has eaten some malteasers!`);
              await wait(1000);
              ...
              setAnnouncerMessage(`${attacker.name} has recovered some vitality.`);
              turn === 0
                ? setPlayerHealth(h =>
